...
async function buildPdf(
  ...
  function drawPageHeader(p: ReturnType<typeof pdfDoc.addPage>): void {
    ...
    p.drawText('SAGUARO CRM', { x: MARGIN, y: PAGE_H - HEADER_H + 22, size: 16, font: boldFont, color: GOLD });
    p.drawText(title, { x: MARGIN + 140, y: PAGE_H - HEADER_H + 22, size: 13, font: boldFont, color: WHITE });
  ...
  function drawFooter(p: ReturnType<typeof pdfDoc.addPage>, pNum: number): void {
    ...
    p.drawText(`Generated by Saguaro CRM — ${dateStr}`, { x: PAGE_W / 2 - 100, y: 6, size: 8, font, color: DARK });
  }
