...
function MeetingsPage() {
  ...
  const renderList = () => (
    <div>
      ...
      {/* New meeting button */}
      <button onClick={openCreate} style={{ ...btnStyle(GOLD), width: '100%', justifyContent: 'center', marginBottom: 16, color: '#000' }}>
        + New Meeting
      </button>
      ...
      {loading ? (
        ...
        filtered.map(m => (
          <div key={m.id} onClick={() => openDetail(m)} style={{ ...cardStyle, cursor: 'pointer', transition: 'border-color .15s' }}>
    ...
    return (
      <div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' as const }}>
          ...
          <button onClick={() => handleEmailMinutes(m.id)} style={btnStyle(GOLD, true)}>Email Minutes</button>
          {m.status === 'Scheduled' && (
