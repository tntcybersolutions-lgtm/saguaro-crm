...
const GOLD='#C8960F',BASE='#F8F9FB',TEXT='#F0F4FF',DIM='#8BAAC8',GREEN='#22C55E',BLUE='#3B82F6',RED='#EF4444',BORDER='#1E3A5F';
const glass:React.CSSProperties={background:'rgba(26,31,46,0.85)',backdropFilter:'blur(16px)',WebkitBackdropFilter:'blur(16px)',border:'1px solid #EEF0F3',borderRadius:16};
...
<div style={{height:8,background:'#EEF0F3',borderRadius:6,overflow:'hidden'}}><div style={{height:'100%',width:avg+'%',background:heatColor(avg),borderRadius:6,transition:'width 0.4s'}}/></div>
...
{loading?(<div style={{display:'flex',flexDirection:'column',gap:8}}>{[1,2,3].map(i=><div key={i} style={{...glass,padding:16,height:80}}><div style={{background:'#F3F4F6',borderRadius:8,height:14,width:'50%',marginBottom:8}}/><div style={{background:'#F8F9FB',borderRadius:6,height:8,width:'100%'}}/></div>)}</div>
...
<div style={{height:5,background:'#EEF0F3',borderRadius:4,overflow:'hidden',marginBottom:10}}><div style={{height:'100%',width:edit.percent_complete+'%',background:heatColor(edit.percent_complete),borderRadius:4,transition:'width 0.2s'}}/></div>
...
<style>{String.raw`input[type=range]{-webkit-appearance:none;height:6px;border-radius:4px;background:#E5E7EB;outline:none}input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:18px;height:18px;border-radius:50%;cursor:pointer}`}</style>
</div>
);
}
