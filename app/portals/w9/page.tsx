      ...
      <div style={{ maxWidth: 440, width: '100%' }}>
        ...
        <div style={{ background: '#F3F4F6', backdropFilter: 'blur(40px)', border: '1px solid #E5E7EB', borderRadius: 16, padding: 32 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input type="text" value={token} onChange={(e) => setToken(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && token.trim() && router.push(`/portals/w9/${token.trim()}`)} placeholder="Paste token from email" style={{ flex: 1, padding: '12px 16px', background: '#EEF0F3', border: '1px solid #E5E7EB', borderRadius: 10, color: '#111827', fontSize: 14, outline: 'none' }} />
            <button onClick={() => token.trim() && router.push(`/portals/w9/${token.trim()}`)} disabled={!token.trim()} style={{ padding: '12px 24px', background: 'linear-gradient(135deg, #C8960F, #C8960F)', border: 'none', borderRadius: 10, color: '#000', fontWeight: 700, fontSize: 14, cursor: 'pointer', opacity: !token.trim() ? 0.5 : 1 }}>Submit</button>
          </div>
        </div>
