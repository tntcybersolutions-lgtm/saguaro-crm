    ...
    <div style={{ minHeight: '100vh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ maxWidth: 440, width: '100%' }}>
        ...
        <div style={{ background: '#F3F4F6', backdropFilter: 'blur(40px)', border: '1px solid #E5E7EB', borderRadius: 16, padding: 32 }}>
          <div style={{ marginBottom: 24 }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '12px 16px', background: '#EEF0F3', border: '1px solid #E5E7EB', borderRadius: 10, color: '#111827', fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
            />
            <button
              onClick={handleEmailAccess}
              disabled={loading || !email.trim()}
              style={{ width: '100%', marginTop: 12, padding: '12px', background: 'linear-gradient(135deg, #C8960F, #C8960F)', border: 'none', borderRadius: 10, color: '#000', fontWeight: 700, fontSize: 14, cursor: loading ? 'wait' : 'pointer', opacity: !email.trim() ? 0.5 : 1 }}
            >
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <input
                ...
                style={{ flex: 1, padding: '12px 16px', background: '#EEF0F3', border: '1px solid #E5E7EB', borderRadius: 10, color: '#111827', fontSize: 14, outline: 'none' }}
              />
              <button
                onClick={handleTokenAccess}
                disabled={!token.trim()}
                style={{ padding: '12px 20px', background: '#E5E7EB', border: '1px solid #D1D5DB', borderRadius: 10, color: '#111827', fontWeight: 600, fontSize: 14, cursor: 'pointer', opacity: !token.trim() ? 0.5 : 1 }}
              >
                Go
              </button>
            </div>
        ...
        <div style={{ textAlign: 'center', marginTop: 24 }}>
          <a href="/" style={{ color: '#6B7280', fontSize: 13, textDecoration: 'none' }}>Back to Saguaro</a>
        </div>
      </div>
    </div>
  );
