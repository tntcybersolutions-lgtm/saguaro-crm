...
export default function PortalsHub() {
  const portals = [
    { title: 'Owner / Client Portal', desc: 'View project progress, budgets, photos, and documents', href: '/portals/client', color: '#C8960F' },
    ...
    <div style={{ minHeight: '100vh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ maxWidth: 600, width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          ...
          <h1 style={{ color: '#111827', fontSize: 32, fontWeight: 700, margin: '0 0 8px' }}>Portal Access</h1>
          <p style={{ color: '#6B7280', fontSize: 14, margin: 0 }}>Select your portal below to get started.</p>
        </div>
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          {portals.map((p) => (
            <a key={p.href} href={p.href} style={{
              ...
              background: '#F3F4F6', backdropFilter: 'blur(40px)',
              border: '1px solid #E5E7EB', borderRadius: 16, padding: '24px 28px',
              borderLeft: `4px solid ${p.color}`,
            }}>
              <div style={{ color: '#111827', fontSize: 18, fontWeight: 700, marginBottom: 4 }}>{p.title}</div>
              <div style={{ color: '#6B7280', fontSize: 13 }}>{p.desc}</div>
            </a>
          ))}
        </div>

        <div style={{ textAlign: 'center', marginTop: 32 }}><a href="/" style={{ color: '#6B7280', fontSize: 13, textDecoration: 'none' }}>Back to Saguaro</a></div>
      </div>
    </div>
  );
}
