...
export default function SubPortalLanding() {
  ...
  const handleTokenAccess = () => {
    ...
    router.push(`/portals/sub/${token.trim()}`);
  };
  ...
  const handleEmailAccess = async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/portal/sub/auth', {
        method: 'POST',
      ...
      if (res.ok && data.token) {
        router.push(`/portals/sub/${data.token}`);
      } else {
        setError(data.error || 'No portal access found. Contact your GC.');
      }
    } catch {
  ...
  return (
    <>
    <PortalHeader portalName="Subcontractor Portal" />
